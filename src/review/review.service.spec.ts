...
import { MailerModule, Mailer } from 'nestjs-mailer';

// Mock for MailerService
class MockMailerService {
  sendMail(options: any) {
    // Simulate mail sending logic, if necessary
...
describe('ReviewService', () => {
  ...
  beforeEach(async () => {
    ...
    mockHelperService = {
      ...
      changeDocumentStatus: jest.fn().mockResolvedValue(null),
		...
		mockMailerService = new MockMailerService();
      ...
      providers: [
        ...
        },
        {
          ...
          useValue: mockHelperService,
				...
				{
          provide: "default_MailerModuleConnectionToken",
  ...
  describe('assignReviewer', () => {
    ...
    it('Throw if trying to assign yourself as reviewer', async () => {
      await expect(
        service.assignReviewer({ ...user, id: 'user2' }, documentId, body),
      ).rejects.toThrow(
        new ForbiddenException('You cannot assign yourself as a reviewer'),
      );
    });
    ...
    it('Throw if reviewer not found', async () => {
      ...
      await expect(
        service.assignReviewer(user, documentId, body),
    ...
    it('Successfully assign a reviewer', async () => {
      ...
      jest
        .spyOn(mockReviewRepository, 'save')
    ...
    it('Throws error if ownership check fails', async () => {
      jest
        .spyOn(mockHelperService, 'checkOwnership')
    ...
    it('Throws error if there is already an existing review in "wait" status', async () => {
      jest
        ...
        .mockResolvedValueOnce(new Review());
    ...
    it('Throws error if owner tries to pass review', async () => {
      jest
        .spyOn(mockHelperService, 'checkIsReviewerOrOwner')
        .mockResolvedValue('owner');

      await expect(
        service.passReview(user, documentId, { comment: 'Looks good' }),
